...
module.exports = S => {
  ...
  return class Offline extends S.classes.Plugin {
    ...
    /**
     * Get or create a webpack runtime - singleton
     *
     * @returns {*}
     */
    getWebpackRuntime() {
          ...
          this.server.route({
            ...
            handler: (request, reply) => { // Here we go
              ...
              const getHandler = () => {
                if (funRuntime === 'webpack') {
                  ...
                  try {
                    ...
                    let handler = require(handlerPath)[handlerParts[1]];
              ...
              getHandler().then(handler => {
                serverlessLog(`Got handler - keys ${Object.keys(handler)}`);
